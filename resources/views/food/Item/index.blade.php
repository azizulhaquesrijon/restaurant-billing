...
@section('page-header')
    <i class="fa fa-list"></i> Food Item
@stop
@section('css')
...
@section('content')
    ...
    <div class="page-header">
      <a class="btn btn-xs btn-info" href="{{ route('food.create') }}" style="float: right; margin: 0 2px;"> <i
                        ...
                        @foreach($item as $items )
                            <tr>
                              <tr>
                                ...
                                <td>{{$items->price}}</td>
                                <td>{{$items->vat}}</td>
                                <td>{{$items->foodcode}}</td>

                                <td><button type="button" class="btn btn-primary" onclick="window.location='{{ Route("food.edit",$items->id) }}'">Update</button></td>
                                <td><button type="button" class="btn btn-danger" onclick="delete_check({{ $items->id }})">Delete</button></td>
                              </tr>
                              <form action="{{ route('food.destroy',$items->id)}}"
                                  id="deleteCheck_{{ $items->id}}" method="POST">
                                  @csrf
                                  @method("DELETE")
                              </form>
                        @endforeach
